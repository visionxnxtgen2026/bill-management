  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-3">
        ...
        <button className="btn-secondary" onClick={() => exportToCsv('out-of-stock.csv', filtered)} disabled={filtered.length === 0}>
          <Download className="w-4 h-4" /> Export
        </button>
      </div>

      {filtered.length === 0 ? (
        <EmptyState title="No out-of-stock items" message="Nothing has run out." />
      ) : (
